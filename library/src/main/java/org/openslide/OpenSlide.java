...
public final class OpenSlide implements Closeable {
    ...
    public OpenSlide(File file) throws IOException {
        ...
        tileSize = OpenSlideJNI.openslide_get_tile_size(osr);
...
//            int data[] = ((DataBufferInt) img.getRaster().getDataBuffer())
//                    .getData();
            byte[] bytes = OpenSlideJNI.openslide_read_associated_image(osr);
...
//            checkError();
    ...
    public byte[] readRawTile(long tx, long ty, int level) throws IOException {
        byte[] tile = OpenSlideJNI.openslide_read_raw_tile(osr, tx, ty, level);
        if (tile.length == 0) {
            throw new IOException("failed to read raw tile");
        }
        return tile;
    }
