...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static void writeLines(final String fileName, final Collection<?> lines,
		...
		try {
			FileUtils.writeLines(new File(fileName), "utf-8",lines, append);
			logger.debug("文件 " + fileName + " 写入成功!");
		} catch (IOException e) {
			logger.debug("文件 " + fileName + " 写入失败! " + e.getMessage());
		}
	...
	public static void writToFile(Object jsonObject,String fileName){
		//写入备注文件里
		String timeFile = DateUtils.formatDate(new Date());
		String path = PropertyUtil.getProperty("logging.predict")+"/"+fileName+"-"+timeFile+".json";
		File file = new File(path);
		if (!file.exists()){
			FileUtils.createFile(path);
		}
		String finalResult = jsonObject.toString();
		String time = DateUtils.formatDateTime(new Date());
		String message = time + " -- "+finalResult;
